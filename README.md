...
- **Actions** -- delete, restart (rollout restart for StatefulSets), edit YAML in `$EDITOR`, exec into a pod (requires `kubectl` on `$PATH`)
...
| `x` | Exec into pod (via `kubectl exec -it`) |
| `/` | Filter by name |
